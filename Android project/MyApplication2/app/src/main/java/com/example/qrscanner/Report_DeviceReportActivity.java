...
/**
 * デバイスのQRコードをスキャンし、結果画面へ遷移する
 */
public class Report_DeviceReportActivity extends AppCompatActivity {
    ...
    // --- Business: Scan D code and submit ---
    ...
    private final Runnable clearCacheRunnable = () -> {
        deviceCode = null;
        runOnUiThread(() -> tvResult.setText("スキャンを待機中…"));
    };
    ...
    @Override protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_device_report); // Use existing lend layout for scanning
    ...
    // --- Business: Classify D, submit, and stop ---
    ...
    private void handleScannedText(String raw) {
        ...
        if (isDevice(text)) {
            ...
            return; // Invalid prefix, ignore
        }

        if (changed) {
            runOnUiThread(() -> {
                tvResult.setText("デバイス: " + deviceCode);
            });
        }
    ...
    private void maybeNavigateToResult() {
        if (deviceCode != null) {
            ...
            Intent intent = new Intent(this, Report_DeviceReportActivity.class);
            // Pass the scanned device code to the next activity
